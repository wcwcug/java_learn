...
public class Rectangle implements Cloneable{//边界矩形
	private Point low;  
	private Point high;  
    /*叶子结点连接下层R树*/
    RTree lowtree; 
...
//        high = (Point) p2.clone();    
     ...
     */  
    public Rectangle getUnionRectangle(Rectangle rectangle)  //包围两个Rectangle的最小Rectangle 
    {  
        if(rectangle == null)  
        ...
        {  
            min[i] = Math.min(low.getFloatCoordinate(i), rectangle.low.getFloatCoordinate(i));  //左下角结点
            max[i] = Math.max(high.getFloatCoordinate(i), rectangle.high.getFloatCoordinate(i)); //右上角结点 
        }  
          
        return new Rectangle(new Point(min), new Point(max));  //返回最小Rectangle 
    }  
