...
public abstract class RTNode { //抽象类
    ...
    {  
        if(datas[i + 1] != null)  
        {  
            System.arraycopy(datas, i + 1, datas, i, usedSpace - i -1);  //数组间的复制
    ...
    {  
        ...
        {  
            ...
            int min = Math.round(rtree.getNodeCapacity() * rtree.getFillFactor());  //四舍五入
            if(usedSpace < min)  
            {  
     ...
     * @return 两个组中的条目的索引 
     */  
    protected int[][] quadraticSplit(Rectangle rectangle)  // 分裂结点的平方算法
    {  
        if(rectangle == null)  
        {  
        ...
        datas[usedSpace] = rectangle;   //先将rectangle添加进rectangle组  
        int total = usedSpace + 1;      //结点总数加1 
